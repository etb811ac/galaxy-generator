...
const createGalaxy = () => {
    ...
    gsap.to(points.rotation, { y: parameters.spin < 0 ? -Math.PI * 2: Math.PI * 2, duration: Math.abs(parameters.spin) * 20, repeat: -1, ease: 'none' })
}

createGalaxy()


//gui panel initialice
gui.add(parameters, 'count').min(100).max(100000).step(100).onFinishChange(createGalaxy)
gui.add(parameters, 'size').min(0.001).max(0.1).step(0.001).onFinishChange(createGalaxy)
gui.add(parameters, 'radius').min(0.001).max(20).step(0.01).onFinishChange(createGalaxy)
...
if (window.innerWidth < 400) gui.close()
...
gsap.from(camera.position, {x: 0, y:0, z: 0, duration:10})
